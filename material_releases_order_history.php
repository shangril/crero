...
function processdata ($data){
	...
	if (is_array($data)){
		...
		foreach ($keys as $key){
			echo '<li>'.htmlspecialchars(html_entity_decode(html_entity_decode($key))).'</li>';
			processdata($data[$key]);
		}
	}
	else {
		echo '<li>'.htmlspecialchars(html_entity_decode($data)).'</li>';
...
if (file_exists('./d/orders')&&is_dir('./d/orders')){
	...
	foreach ($orders as $bill){
		if (strstr('./d/orders/'.$bill, '.php')){
			rename ('./d/orders/'.$bill, './d/orders/'.str_replace ('.php', '.dat', $bill)); 
			$bill=str_replace ('.php', '.dat', $bill);
		}
